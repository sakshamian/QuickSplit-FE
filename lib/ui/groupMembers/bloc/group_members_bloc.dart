...
class GroupMembersBloc extends Bloc<GroupMembersEvent, GroupMembersState> {
  ...
  Future<void> _onFetchGroupMembers(
      FetchGroupMembers event, Emitter<GroupMembersState> emit) async {
  ...
  Future<void> _onRemoveMember(
      RemoveMember event, Emitter<GroupMembersState> emit) async {
    try {
      ...
      if (result.containsKey('message')) {
        ...
        add(FetchGroupMembers(
            groupId: event.groupId)); // Refresh the group members
      } else {
        ToastService.showToast('Error: ${result['error']}');
        // emit(GroupMembersError('Failed to remove member: ${result['error']}'));
      }
    } catch (error) {
      ToastService.showToast('Failed to remove member: $error');
